...
bool AdjustmentHandler::IsPlayerProne()
{
	...
	auto graph = process->animationVariableCache->GetAnimationGraph();
	if (!graph) {
		return false;
	}

	auto isProne = 0;
	if (!graph->GetGraphVariableInt("IsCrawling", isProne)) {
		return false;
	}
...
bool AdjustmentHandler::CheckEnoughSpaceToStand(RE::ActorHandle a_actorHandle)
{
	...
	if (AdjustmentHandler::IsPlayerProne()) {
